...
describe("Charity contracts integration", function () {
  ...
  async function deployCharityFixture() {
    ...
    const CharityReputation = await ethers.getContractFactory(
      "CharityReputation"
    );
    const reputation = await CharityReputation.deploy(
      await governance.getAddress()
    );
    ...
    const registerCharity = async (signer, name, metaCID) => {
      return await registry
        .connect(signer)
        ["registerCharity(string,string)"](name, metaCID);
    };
    ...
    const getProfile = async (orgId) => {
      ...
      if (
        profile.registrant === zeroAddr ||
        profile.registrant === ethers.ZeroAddress ||
        ...
        const error = Object.assign(
          new Error("execution reverted: Charity not found"),
          {
    ...
    const getTreasuryBalance = async (orgId) => {
      ...
      return [
        treasuryData.totalBalance,
        treasuryData.availableBalance,
        treasuryData.lockedBalance,
      ];
    };
  ...
  describe("1. CharityRegistry", function () {
    it("1a) should register charity with valid inputs and emit event", async () => {
      const { registry, charityOwner, registerCharity } = await loadFixture(
        deployCharityFixture
      );

      await expect(registerCharity(charityOwner, "Charity A", "QmMeta"))
        .to.emit(registry, "CharityRegistered")
        .withArgs(1n, "Charity A", await charityOwner.getAddress());

      const orgId = await registry.addressToOrgId(
        await charityOwner.getAddress()
      );
    ...
    it("1d) should allow multiple charities with different owners", async () => {
      ...
      const orgId1 = await registry.addressToOrgId(
        await charityOwner.getAddress()
      );
      const orgId2 = await registry.addressToOrgId(
        await charityOwner2.getAddress()
      );
    ...
    it("1e) should enforce onlyAdmin for setApproval", async () => {
      const { registry, charityOwner, deployer, user1, registerCharity } =
        await loadFixture(deployCharityFixture);
    ...
    it("1i) should reject unauthorized profile updates", async () => {
      const { registry, charityOwner, charityOwner2, user1, registerCharity } =
        await loadFixture(deployCharityFixture);
    ...
    it("1k) should reject operations on non-existent charity", async () => {
      const { registry, treasury, deployer, getProfile } = await loadFixture(
        deployCharityFixture
      );
      ...
      await expect(
        registry
          .connect(deployer)
          .setTreasury(999n, await treasury.getAddress())
      ).to.be.revertedWith("Charity not found");

      // Test getProfile revert - use direct call if available, otherwise test fallback behavior
      if (typeof registry.getProfile === "function") {
        await expect(registry.getProfile(999n)).to.be.revertedWith(
          "Charity not found"
        );
      } else {
        // Fallback: test that getProfile helper correctly throws for non-existent charity
        await expect(getProfile(999n)).to.be.rejectedWith("Charity not found");
      }
    });
  });
  ...
  describe("2. CharityReputation", function () {
    it("2a) should initialize reputation with default score", async () => {
      const { registry, reputation, charityOwner, deployer, registerCharity } =
        await loadFixture(deployCharityFixture);
    ...
    it("2c) should enforce onlyAdmin for initialization", async () => {
      const { registry, reputation, charityOwner, user1, registerCharity } =
        await loadFixture(deployCharityFixture);
    ...
    it("2d) should reject operations on non-initialized org", async () => {
      const { registry, reputation, charityOwner, oracle, registerCharity } =
        await loadFixture(deployCharityFixture);
    ...
    it("2i) should cap score at MAX_SCORE", async () => {
      ...
      for (let i = 1; i <= 60; i++) {
        await reputation
          .connect(oracle)
          .updateOnEventOutcome(orgId, BigInt(i), true);
      }
    ...
    it("2j) should cap score at MIN_SCORE", async () => {
      ...
      for (let i = 1; i <= 30; i++) {
        await reputation
          .connect(oracle)
          .updateOnEventOutcome(orgId, BigInt(i), false);
      }
    ...
    it("2m) should return correct reputation tier", async () => {
      ...
      for (let i = 1; i <= 50; i++) {
        await reputation
          .connect(oracle)
          .updateOnEventOutcome(orgId2, BigInt(i), true);
      }
  ...
  describe("3. CharityTreasury", function () {
    async function setupTreasuryFixture() {
      ...
      await treasury
        .connect(deployer)
        .createTreasury(orgId, await charityOwner.getAddress());

      return { ...fixture, orgId, getTreasuryBalance };
    }

    it("3a) should create treasury with correct initial state", async () => {
      const { treasury, charityOwner, deployer } = await loadFixture(
        deployCharityFixture
      );
      ...
      await expect(
        treasury
          .connect(deployer)
          .createTreasury(1n, await charityOwner.getAddress())
      )
    ...
    it("3b) should reject invalid treasury creation", async () => {
      const { treasury, deployer, charityOwner } = await loadFixture(
        deployCharityFixture
      );
      ...
      await expect(
        treasury
          .connect(deployer)
          .createTreasury(0n, await charityOwner.getAddress())
    ...
    it("3c) should enforce onlyAdmin for treasury creation", async () => {
      const { treasury, charityOwner, user1 } = await loadFixture(
        deployCharityFixture
      );
...
it("3d) should confirm funds received from oracle and update balances", async () => {
    ...
    it("3i) should reject withdrawal with invalid inputs", async () => {
      ...
      await expect(
        treasury
          .connect(charityOwner)
          .withdraw(ethers.ZeroAddress, ethers.parseEther("10"))
      ).to.be.revertedWith("Invalid recipient address");

      await expect(
        treasury
          .connect(charityOwner)
          .withdraw(await beneficiary.getAddress(), 0n)
      ).to.be.revertedWith("Amount must be positive");

      await expect(
        treasury
          .connect(charityOwner)
          .withdraw(await beneficiary.getAddress(), ethers.parseEther("200"))
    ...
    it("3m) should deactivate treasury correctly", async () => {
      const { treasury, deployer, orgId } = await loadFixture(
        setupTreasuryFixture
      );

      await expect(treasury.connect(deployer).deactivateTreasury(orgId))
  ...
  describe("4. CharityEvent", function () {
    async function setupEventFixture() {
      ...
      const eventCtr = await createEvent(
        charityOwner,
        orgId,
    ...
    it("4b) should reject invalid event creation", async () => {
      const { registry, governance, charityOwner, registerCharity } =
        await loadFixture(deployCharityFixture);
    ...
    it("4d) should reject updateRaised when not in FUNDING phase", async () => {
      const { eventCtr, charityOwner, deployer } = await loadFixture(
        setupEventFixture
      );
    ...
    it("4e) should allow owner to close funding", async () => {
      ...
      await expect(eventCtr.connect(charityOwner).closeFunding()).to.emit(
        eventCtr,
        "PhaseChanged"
      );
    ...
    it("4f) should reject closeFunding from non-owner", async () => {
      ...
      await expect(eventCtr.connect(user1).closeFunding()).to.be.revertedWith(
        "Not charity owner"
      );
    });
    ...
    it("4g) should allow owner to submit evidence in CLOSED phase", async () => {
      ...
      await expect(eventCtr.connect(charityOwner).submitEvidence("QmEvidence"))
    ...
    it("4h) should reject submitEvidence with invalid inputs", async () => {
      const { eventCtr, charityOwner, user1 } = await loadFixture(
        setupEventFixture
      );
    ...
    it("4j) should allow oracle to set verification status", async () => {
      const { eventCtr, charityOwner, oracle } = await loadFixture(
        setupEventFixture
      );
    ...
    it("4p) should allow admin to cancel event", async () => {
      ...
      await expect(eventCtr.connect(deployer).cancel()).to.emit(
        eventCtr,
        "Cancelled"
      );

      const summary = await eventCtr.getEventSummary();
      expect(summary[2]).to.equal(6n); // CANCELLED
    });

    it("4q) should check goal reached correctly", async () => {
      const { eventCtr, deployer, getEventGoalReached } = await loadFixture(
        setupEventFixture
      );
    ...
    it("4r) should check deadline passed correctly", async () => {
      const { eventCtr, advanceTime, getEventDeadlinePassed } =
        await loadFixture(setupEventFixture);
    ...
    it("4s) should handle pause correctly", async () => {
      const { eventCtr, pauser, deployer } = await loadFixture(
        setupEventFixture
      );
...
describe("5. Integration Tests", function () {
    it("5a) full flow: register -> approve -> create treasury -> fund -> verify -> release", async () => {
      ...
      await treasury
        .connect(charityOwner)
        .withdraw(await beneficiary.getAddress(), ethers.parseEther("50"));
      expect(await sgd.balanceOf(await beneficiary.getAddress())).to.equal(
