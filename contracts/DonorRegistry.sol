...
contract DonorRegistry is Registry {
     ...
     */
    function registerDonor(string calldata _name, string calldata _metaCID) 
        external 
        notRegistered
        whenNotPaused
        whenSystemNotPaused
     ...
     */
    function updateProfile(string calldata _metaCID) 
        external 
        onlyRegisteredDonor
        whenNotPaused
        whenSystemNotPaused
     ...
     */
    function setVerification(address _donor, bool _verified) 
        external 
        onlyAdmin
        whenNotPaused
        whenSystemNotPaused
    {
        ...
        bool wasVerified = profiles[_donor].isVerified;
        profiles[_donor].isVerified = _verified;
    ...
    /**
     * @dev Check if a donor is registered (alternative naming for compatibility)
     * @param donor Address of the donor
     * @return Whether the donor is registered
     */
     ...
     */
    function isVerified(address _donor) external view returns (bool) {
        return profiles[_donor].isVerified;
    }
     ...
     * @param _donor Address of the donor
     */
    function getProfile(address _donor) 
        external 
        view 
        returns (
            address donorAddress,
